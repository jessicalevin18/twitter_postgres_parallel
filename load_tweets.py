...
def insert_tweet(connection,tweet):
    ...
    with connection.begin() as trans:
        ...
        if 'updated_at' in tweet:
            if tweet['updated_at'] is None:
                updated_at = None
            else:
                updated_at = get_updated_at(tweet['updated_at'], connection)
        else:
            updated_at = None

        if 'withheld_in_countries' in tweet:
            if tweet['withheld_in_countries'] is None:
        ...
        sql = sqlalchemy.sql.text('''
            INSERT INTO users
                (id_users, created_at, updated_at, id_urls, friends_count, listed_count, favourites_count,
                statuses_count, protected, verified, screen_name, name, location, description, withheld_in_countries)
                VALUES
                (:id_users, :created_at, :updated_at, :id_urls, :friends_count,
                :listed_count, :favourites_count, :statuses_count, :protected,
                :verified, :screen_name, :name, :location, :description, :withheld_in_countries)
        ...
        sql = sqlalchemy.sql.text(
            ...
            ON CONFLICT (id_users) DO NOTHING
        '''
        # connection.execute(sql, {'id_users': tweet['user']['id']})
        ...
        sql=sqlalchemy.sql.text('''
          INSERT INTO tweets
            (id_tweets, id_users, created_at, in_reply_to_status_id, in_reply_to_user_id, quoted_status_id, retweet_count,
            favorite_count, quote_count, withheld_copyright, withheld_in_countries, source, text, country_code, state_code,
            lang, place_name, geo)
            VALUES
            (:id_tweets, :id_users, :created_at, :in_reply_to_status_id, :in_reply_to_user_id, :quoted_status_id, :retweet_count,
            :favorite_count, :quote_count, :withheld_copyright, :withheld_in_countries, :source, :text, :country_code, :state_code,
        ...
        res = connection.execute(sql,{
            ...
            'place_name': remove_nulls(tweet.get('place', {}).get('name') if tweet.get('place') is not None else None),
            'geo': None if geo_str or geo_chords is None else geo_str + geo_coords
            })
        ...
        tags = (
            ['#' + h['text']  for h in hashtags] +
            ['$' + c['text'] for c in cashtags]
        )
        for tag in tags:
            sql=sqlalchemy.sql.text('''
